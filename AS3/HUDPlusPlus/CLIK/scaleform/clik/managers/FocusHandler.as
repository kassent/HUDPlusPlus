...
package scaleform.clik.managers {
    ...
    // Protected Properties:
        ...
        /** Array of references to what holds focus within the CLIK FocusHandler framework. */
        protected var currentFocusLookup:Array;
        /** Array of references to what the FocusHandler believes "stage.focus" currently references. */
        protected var actualFocusLookup:Array; // NFM: This commonly will point to an InteractiveObject while stage.focus == null.
        
        protected var ignoreFocusChange:Boolean = false;
    ...
    // Public Methods:
        public function getFocus(index:uint):DisplayObject { return currentFocusLookup[index] as DisplayObject; }
        public function setFocus(focus:DisplayObject, index:uint = 0, mouseChange:Boolean = false):void {
            // trace("\n *** FocusHandler :: setFocus( " + focus + " )");
            ...
            if (focusComponent != null) {
                if (!focusComponent.focusable) { focus = null; }
            }
            ...
            // Make focus change
            var actualFocus:DisplayObject = actualFocusLookup[index];
            var currentFocus:DisplayObject = currentFocusLookup[index];  //LM: DisplayObjects? Maybe UIComponents....
            ...
            if (currentFocus != focus) {
                ...
                if (focusComponent != null) { 
                    focusComponent.focused = focusComponent.focused & ~(1 << index);
                }
                ...
                // Turn on new focus.
                currentFocus = focus;
                currentFocusLookup[index] = focus;
                focusComponent = currentFocus as UIComponent;
                if (focusComponent != null) {
                    focusComponent.focused = focusComponent.focused | (1 << index);
                }
            ...
            if (actualFocus != currentFocus && (!isActualFocusTextField || (isActualFocusTextField && !isCurrentFocusUIComponent))) {
                /*var controllerMask:Number = 0;//Selection.getControllerMaskByFocusGroup(index);
                ...
                for (var i:uint=0; i<numControllers; i++) {
                    ...
                    if (controllerValue) { 
                        _stage.focus = focus as InteractiveObject; //LM: What about the controller index?
                        //Selection.setFocus(currentFocus, i);
                    }
                }*/
                //trace("> Actual focus is different from final focus");
                ...
                if (focusParam is TextField && focusParam != focus && focus == null) {
                    ...
                    _stage.focus = focusParam as InteractiveObject;
                } else {
                    preventStageFocusChanges = true;
                    _stage.focus = focus as InteractiveObject;
                }
    ...
    // Protected Methods:
        protected function handleInput(event:InputEvent):void {
            ...
            var component:InteractiveObject = currentFocusLookup[index];
            ...
            // var focusedElement:InteractiveObject = _stage.focus; //LM: Need proper index.
            var focusedElement:InteractiveObject = currentFocusLookup[index];  // NFM: Use our currentFocusLookup rather than _stage.focus since it may be null?
            // Get what we THINK is the stage-focused element
            var actualFocus:InteractiveObject = actualFocusLookup[index]
            var stageFocusedElement:InteractiveObject = _stage.focus;
    
            // TextField edge case
            if (actualFocus is TextField && actualFocus == focusedElement && handleTextFieldInput(nav, index)) { return; }
            if (actualFocus is TextField && handleTextFieldInput(nav, index)) { return; }
            ...
            trace("_stage.focus: \t\t" + _stage.focus);
            trace("currentFocusLookup["+index+"]: \t" + currentFocusLookup[index]);
            trace("actualFocusLookup["+index+"]: \t" + actualFocusLookup[index]);
            */
            ...
            if (focusedElement == null) { 
                if (_stage.focus && _stage.focus is UIComponent) {
                    focusedElement = _stage.focus as UIComponent;
                }
            }
            ...
            if (actualFocus is TextField && actualFocus.parent == focusedElement) { 
                focusedElement = _stage.focus;
            }
            
            // Change focus manually.
            var newFocus:InteractiveObject = FocusManager.findFocus(nav, /*focusContext,*/ null, focusMode == FocusMode.LOOP, focusedElement, false, index);
        ...
        protected function handleFocusChange(oldFocus:InteractiveObject, newFocus:InteractiveObject, event:FocusEvent):void {
            ...
            // Do not allow the textField -> null default behaviour in the framework.
            if (oldFocus is TextField && newFocus == null) { 
                event.preventDefault(); //LM: Does this work?
                return; 
            }
            
            var sfEvent = event as FocusEventEx;
            var controllerIndex = sfEvent == null ? 0 : sfEvent.controllerIdx;
            var index:uint = 0;// Selection.getControllerFocusGroup(controllerIdx); //LM: Not implemented yet
            ...
            //LM: Is this still necessary? NOT IMPLEMENTED YET
            /*
            ...
            */
            ...
            // Storing stage focus
            actualFocusLookup[index] = newFocus;
            setFocus(newFocus, index, (event.type == FocusEvent.MOUSE_FOCUS_CHANGE));
        }
        ...
        protected function updateActualFocus(event:FocusEvent):void {
            ...
            // targets.
            var currentFocus:InteractiveObject = currentFocusLookup[index];
            if (newFocus != null && newFocus is TextField && newFocus.parent != null && 
                currentFocus == newFocus.parent && currentFocus == oldFocus) { 
                return;
            }
            
            // NFM: Allows users to use _stage.focus = rather than FocusHandler.setFocus().
            ...
            if (newFocus != currentFocus) {
                ...
                if (!(isActualFocusTextField && isCurrentFocusUIComponent) || newFocus == null) {
                    if (!preventStageFocusChanges || isActualFocusTextField) { 
                        setFocus(newFocus); // Update the framework focus manually.
                    }
                }
            }
        }
        ...
        protected function handleTextFieldInput(nav:String, controllerIdx:uint):Boolean {
            var actualFocus:TextField = actualFocusLookup[controllerIdx] as TextField;
            ...
            //LM: Old code looked up the actualFocus via the focusIdx.
