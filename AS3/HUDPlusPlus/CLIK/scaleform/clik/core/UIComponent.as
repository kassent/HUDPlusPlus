...
package scaleform.clik.core {
    ...
    // Initialization:
        ...
        protected function addedToStage(event:Event):void {
            ...
            if ( enableInitCallback && Extensions["CLIK_addedToStageCallback"] ) {
                var tName:String = this.name as String;
                var tPath:String = CLIK.getTargetPathFor(this);
    ...
    // Public Getter / Setters:
        ...
        public function set focused(value:Number):void {
            ...
            // LM: Implement
            /*
            var l:int = 1;//Selection.numFocusGroups;
            for (var i:uint=0; i<l; i++) {
                var focused:Boolean = Boolean(_focused >> i & 0x1);
                if (focused) {
                    var controllerMask:Number = 1;//Selection.getControllerMaskByFocusGroup(i);
                    var numControllers:uint = 1; //Capabilities.numControllers;
                    for (var j:uint=0; j<numControllers; j++) {
                        var controllerValue:Boolean = Boolean(controllerMask >> j & 0x1);
                        if (controllerValue && Selection.getFocus(j) == this) {
                            stage.focus = null; //Selection.setFocus(null, j);
                        } else {
                            stage.focus = this; //Selection.setFocus(this, j); //LM: Needs controllerIndex.
                        }
                    }
                }
            }
            */
            ...
            // Will have to add multi-controller support (above, commented out)
            // LM: Consider using FocusHandler methods to avoid a bunch of focus loop
    ...
    // Public Methods:
        ...
        public function setActualSize(newWidth:Number, newHeight:Number):void {
            ...
            if (super.width != newWidth || _width != newWidth) {
                ...
                // This seems crazy, but without BOTH of these lines, super.width is not updated when the clip is rotated in Flash Player.
                // super.width = newWidth; // May not be necessary with the scaleY fix in ScrollBar.drawLayout().
            }
            
            if (super.height != newHeight || _height != newHeight) {
                super.height = _height = newHeight;
                // super.height = newHeight; // See similar super.width call above.
            }
        }
