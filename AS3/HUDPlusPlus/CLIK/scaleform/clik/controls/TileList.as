package scaleform.clik.controls {
    ...
    // Initialization:
        ...
        public function set rowCount(value:uint):void {
            ...
            h = rowHeight;
            height = (h * value) + (margin) + padding.horizontal;
        }
        ...
        public function set columnCount(value:uint):void {
            ...
            w = _columnWidth;
            width = (w * value) + (margin) + padding.horizontal
        }
        ...
        override public function get availableWidth():Number {
            return Math.round(_width) - (margin * 2) - ((_direction == DirectionMode.VERTICAL && _autoScrollBar) ? Math.round(_scrollBar.width) : 0);
        }
        
        /** Retrieve the available height of the component. */
        override public function get availableHeight():Number {
            return Math.round(_height) - (margin * 2) - ((_direction == DirectionMode.HORIZONTAL && _autoScrollBar) ? Math.round(_scrollBar.width) : 0);
        }
        
        /** The scroll position of the list. */
        public function get scrollPosition():Number { return _scrollPosition; }
        public function set scrollPosition(value:Number):void {
            var maxScrollPosition:Number = Math.ceil((_dataProvider.length - _totalRows * _totalColumns) / (_direction=="horizontal" ? _totalRows : _totalColumns)); // @TODO: Seems like this can be cached on invalidate.
