...
package scaleform.clik.controls {
    ...
    // Public getter / setters:
        ...
        public function set selected(value:Boolean):void {
            ...
            if (enabled) { 
                if (!_focused) {
                    setState("toggle");
                } else if (_pressedByKeyboard && _focusIndicator != null) { // Pressed with keyboard + focusIndicator (down->selected_up, selected_down->up)
    ...
    // Protected Methods:
        ...
        protected function handleMousePress(event:MouseEvent):void {
            ...
            if (enabled) { 
                ...
                if (autoRepeat && _repeatTimer == null) {
                    _autoRepeatEvent = new ButtonEvent(ButtonEvent.CLICK, true, false, mouseIdx, false, true);
                    _repeatTimer = new Timer(repeatDelay, 1);
                    _repeatTimer.addEventListener(TimerEvent.TIMER_COMPLETE, beginRepeat, false, 0, true);
                    _repeatTimer.start();
                }
                
                var sfButtonEvent:ButtonEvent = new ButtonEvent(ButtonEvent.PRESS, true, false, mouseIdx, false);
        ...
        protected function handleMouseRelease(event:MouseEvent):void {
            ...
            if (!_isRepeating) {
                var sfButtonEvent:ButtonEvent = new ButtonEvent(ButtonEvent.CLICK, true, false, mouseIdx, false);
                dispatchEvent(sfButtonEvent);
            }
        ...
        protected function handlePress(controllerIndex:uint = 0):void {
            ...
            // Uses single controller like AS2.
            if (autoRepeat && _repeatTimer == null) {
                _autoRepeatEvent = new ButtonEvent(ButtonEvent.CLICK, true, false, controllerIndex, true, true);
                _repeatTimer = new Timer(repeatDelay, 1);
                _repeatTimer.addEventListener(TimerEvent.TIMER_COMPLETE, beginRepeat, false, 0, true);
                _repeatTimer.start();
            }
            
            var sfEvent:ButtonEvent = new ButtonEvent(ButtonEvent.PRESS, true, false, controllerIndex, true);
        ...
        protected function handleRelease(controllerIndex:uint = 0):void {
            ...
            if (!_isRepeating) {
                var sfEvent:ButtonEvent = new ButtonEvent(ButtonEvent.CLICK, true, false, controllerIndex, true);
                dispatchEvent(sfEvent);
            }
